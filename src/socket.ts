    ...
    try {
      ...
      socket.on("send-message", async ({ receiverId, text, file }, callback) => {
        ...
        try {
          ...
          if (!chatList) {
            ...
            chatList = await ChatListServices.createChatListIntoDb(user, receiverId);
          }
      ...
      socket.on("seen", async ({ chatId }, callback) => {
        ...
        try {
          ...
          //----------------------ChatList------------------------//
          const ChatListUser1 = await ChatListServices.getChatListByUserId(user1.toString());

          const ChatListUser2 = await ChatListServices.getChatListByUserId(user2.toString());
